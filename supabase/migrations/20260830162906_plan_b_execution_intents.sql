-- B live reservation preparation. No activation, scheduler, or exchange submission.
create table public.plan_b_execution_intents (
 id bigint generated by default as identity primary key,
 signal_id bigint not null unique references public.plan_b_signals(id),
 client_order_id text not null unique check(client_order_id like 'pb16-%'),
 symbol text not null check(symbol in ('AVAX','ICP','BCH','DOGE','UNI')),
 side text not null check(side in ('long','short')),
 status text not null default 'reserved' check(status in ('reserved','submitted','unknown','partial','open','closing','closed','rejected','expired')),
 quantity numeric not null check(quantity>0), reserved_usd numeric not null check(reserved_usd>0),
 fill_quantity numeric, fill_price numeric, order_id text,
 snapshot_at timestamptz not null, updated_at timestamptz not null default now(),
 created_at timestamptz not null default now()
);
create unique index plan_b_one_unsettled_symbol on public.plan_b_execution_intents(symbol)
 where status not in ('closed','rejected','expired');
alter table public.plan_b_execution_intents enable row level security;
revoke all on public.plan_b_execution_intents from public,anon,authenticated;
grant all on public.plan_b_execution_intents to service_role;
grant usage,select on sequence public.plan_b_execution_intents_id_seq to service_role;
...
declare st public.plan_b_trading_state; item jsonb; sig public.plan_b_signals; held numeric; demand numeric; available numeric; qty numeric; cost numeric; lev numeric;
begin
 select * into st from public.plan_b_trading_state where id='singleton' for update;
 if st.strategy_id<>'b_reserved_margin_stage16' or not st.enabled or st.test_mode then raise exception 'B live execution disabled'; end if;
 ...
 for item in select value from jsonb_array_elements(p_items) loop
  ...
  if not found or sig.strategy_id<>'b_reserved_margin_stage16' or sig.status<>'active' or sig.confirmed_at>clock_timestamp() or sig.entry_deadline<=clock_timestamp() then raise exception 'invalid or expired B signal'; end if;
  qty:=(item->>'quantity')::numeric;cost:=(item->>'reserved_usd')::numeric;
  lev:=case sig.symbol when 'AVAX' then 3 when 'ICP' then 5 when 'BCH' then 3 when 'DOGE' then 5 when 'UNI' then 2 end;
  ...
  insert into public.plan_b_execution_intents(signal_id,client_order_id,symbol,side,quantity,reserved_usd,snapshot_at)
   values(sig.id,'pb16-'||sig.id,sig.symbol,sig.side,qty,cost,p_snapshot);
 end loop;
...
declare n integer;
begin
 perform 1 from public.plan_b_trading_state where id='singleton' and enabled and not test_mode and strategy_id='b_reserved_margin_stage16' for update;
 if not found then return false; end if;
 update public.plan_b_execution_intents i set status='submitted',updated_at=now()
 from public.plan_b_signals s where i.signal_id=s.id and i.client_order_id=p_id and i.status='reserved' and s.entry_deadline>clock_timestamp();
