create or replace function public.plan_b_publish_opportunities(p_expected bigint,p_steps jsonb)
...
begin
 ...
 if prev is null or prev->>'version'<>'b_profit_lock_stage66_v1' then raise exception 'opportunity bootstrap required'; end if;
 ...
 for step in select value from jsonb_array_elements(p_steps) loop
  ...
  if jsonb_typeof(step->'decisions') is distinct from 'object' or (select count(*) from jsonb_object_keys(step->'decisions'))<>12 then raise exception 'all twelve decisions required'; end if;
  ...
  expected:=jsonb_build_object('version','b_profit_lock_stage66_v1','lastConfirmedAt',t,'coreBusyUntil',busy,'nextEligibleAt',nxt);
  ...
  for s in select value from jsonb_array_elements(step->'signals') loop
   ...
   if s->>'strategy_id' is distinct from 'b_profit_lock_stage66' or s->>'side' is distinct from step->'decisions'->rule.symbol->>'side' or (s->>'confirmed_at')::timestamptz is distinct from stamp or (s->>'entry_deadline')::timestamptz is distinct from stamp+interval '5 minutes' or (s->>'expires_at')::timestamptz is distinct from stamp+make_interval(hours=>rule.hold_hours) or (s->>'leverage')::numeric is distinct from rule.leverage or (s->>'hold_hours')::integer is distinct from rule.hold_hours or (s->>'signal_price')::numeric<=0 or (s->>'signal_price') is null or (s->>'signal_price') in ('NaN','Infinity','-Infinity') or (s->>'realized_vol_24h')::numeric<0 or (s->>'realized_vol_24h') in ('NaN','Infinity','-Infinity') then raise exception 'signal metadata mismatch';end if;
   ...
   values('pb66:'||rule.symbol||':'||t||':'||(s->>'side'),rule.symbol,s->>'side',case when clock_timestamp()<stamp+interval '5 minutes' then 'active' else 'expired' end,(s->>'signal_price')::numeric,coalesce((s->>'volume_ratio')::numeric,0),coalesce((s->>'return_1h')::numeric,0),coalesce((s->>'realized_vol_24h')::numeric,0),rule.hold_hours,rule.leverage,1,1,stamp,stamp+interval '5 minutes',stamp+make_interval(hours=>rule.hold_hours),'b_profit_lock_stage66',s->'strategy_params');
  end loop;
...
begin
 ...
 if st.strategy_id not in ('b_reserved_margin_stage16','b_core_sparse_stage26','b_core_bnb_stage45','b_profit_lock_stage66') or not st.enabled or st.test_mode then raise exception 'B live execution disabled';end if;
 ...
 for item in select value from jsonb_array_elements(p_items) loop
  ...
  if st.strategy_id in ('b_core_sparse_stage26','b_core_bnb_stage45','b_profit_lock_stage66') then
   if op is null or op->>'version'<>(case when st.strategy_id='b_profit_lock_stage66' then 'b_profit_lock_stage66_v1' when st.strategy_id='b_core_bnb_stage45' then 'b_core_bnb_stage45_v1' else 'b_core_sparse_stage26_v1' end) or (op->>'lastConfirmedAt')::bigint<>floor(extract(epoch from sig.confirmed_at)*1000)::bigint then raise exception 'B incomplete signal hour';end if;
  ...
  insert into public.plan_b_execution_intents(signal_id,client_order_id,symbol,side,quantity,reserved_usd,snapshot_at) values(sig.id,case when st.strategy_id='b_profit_lock_stage66' then 'pb66-' when st.strategy_id='b_core_bnb_stage45' then 'pb45-' when st.strategy_id='b_core_sparse_stage26' then 'pb26-' else 'pb16-' end||sig.id,sig.symbol,sig.side,qty,cost,p_snapshot);
