...
Deno.serve(async req=>{
 ...
 try{
  ...
  const bx=createBExchange({apiKey:Deno.env.get("PLAN_B_BINGX_API_KEY"),secret:Deno.env.get("PLAN_B_BINGX_SECRET_KEY"),parse:JSONBig({storeAsString:true}).parse,
   liveConfigurationSymbols:body.action==="align_stage45_supplement"&&body.confirm==="approved_20260903_stage45_3x_no_orders"?[body.symbol]:[],
   ...
   configurationAuthorized:async()=>{
    const approvedSupplement=body.action==="align_stage45_supplement"&&body.confirm==="approved_20260903_stage45_3x_no_orders"&&body.symbol==='BNB';
    if(!approvedSupplement&&(body.action!=="align_leverage"||body.confirm!=="align_only_no_orders"||runtime.live_ready))return false;
    const s=await state();if(s.strategy_id!==runtime.strategy_id&&!(approvedSupplement&&s.strategy_id==='b_core_idle_stage35'))return false;
  ...
  if(["align_leverage","align_stage45_supplement"].includes(body.action))return Response.json({ok:true,plan:"B",mode:"configuration_only",result:await bx.alignLeverage(body.symbol)});
