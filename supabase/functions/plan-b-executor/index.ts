...
Deno.serve(async req=>{
 ...
 try{
  ...
  const bx=createBExchange({apiKey:Deno.env.get("PLAN_B_BINGX_API_KEY"),secret:Deno.env.get("PLAN_B_BINGX_SECRET_KEY"),parse:JSONBig({storeAsString:true}).parse,
   liveConfigurationSymbols:body.action==="align_stage35_supplement"&&body.confirm==="approved_20260903_stage35_3x_no_orders"?[body.symbol]:[],
   ...
   configurationAuthorized:async()=>{
    const approvedSupplement=body.action==="align_stage35_supplement"&&body.confirm==="approved_20260903_stage35_3x_no_orders"&&['LINK','DOT','LTC'].includes(body.symbol);
  ...
  if(["align_leverage","align_stage35_supplement"].includes(body.action))return Response.json({ok:true,plan:"B",mode:"configuration_only",result:await bx.alignLeverage(body.symbol)});
