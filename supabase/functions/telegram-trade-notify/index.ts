...
const MDD30_STANDARD="A Stage184 · 5배 단일 + C 급변 방어";
...
const keyboard={keyboard:[["🔵 A 현황","🟣 B 현황"],["🔵 A 기록","🟣 B 기록"],["🧠 R-Lab","📅 주차 현황"],["👥 타인 API 연동"],["🔗 대시보드","❓ 도움말"]],resize_keyboard:true,is_persistent:true};
...
function errorFingerprint(e:any){return [String(e?.source||''),String(e?.status_code??''),String(e?.message||'').slice(0,500)].join('|')}
...
Deno.serve(async req=>{
 ...
 try{
  ...
  if(body.action==="repair_webhook"){
   await tg("setWebhook",{url:URL+"/functions/v1/telegram-bot-webhook",secret_token:webhookSecret(BOT,Deno.env.get("TELEGRAM_WEBHOOK_SECRET")||""),allowed_updates:["message","callback_query"],drop_pending_updates:false});
   const info=await tg("getWebhookInfo");
  ...
  for(const e of errs||[]){
   ...
   if(disposition==='immediate'||disposition==='alert'){const fingerprint=errorFingerprint(e),lastAt=Date.parse(lastSystemErrorAlertedAt||'');if(fingerprint!==lastSystemErrorFingerprint||!Number.isFinite(lastAt)||Date.now()-lastAt>=1800000){await send(`🚨 시스템 오류 감지\n구간: ${e.source}\nHTTP: ${e.status_code??"-"}\n내용: ${String(e.message||"").slice(0,500)}\n시각: ${e.created_at}`);lastSystemErrorFingerprint=fingerprint;lastSystemErrorAlertedAt=new Date().toISOString();}}
  ...
  for(const e of pendingById.values()){
   ...
   else if(disposition==='alert'){const fingerprint=errorFingerprint(e),lastAt=Date.parse(lastSystemErrorAlertedAt||'');if(fingerprint!==lastSystemErrorFingerprint||!Number.isFinite(lastAt)||Date.now()-lastAt>=1800000){await send(`🚨 시스템 오류 감지\n구간: ${e.source}\nHTTP: ${e.status_code??"-"}\n내용: ${String(e.message||"").slice(0,500)}\n시각: ${e.created_at}`);lastSystemErrorFingerprint=fingerprint;lastSystemErrorAlertedAt=new Date().toISOString();}}
  }
 ...
 }catch(e){const detail=errorText(e);console.error("telegram notify failed",stage,detail);return Response.json({ok:false,stage,error:detail},{status:500})}
});
