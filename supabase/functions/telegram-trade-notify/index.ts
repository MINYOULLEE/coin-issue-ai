...
Deno.serve(async req=>{
 ...
 try{
  ...
  for(const x of newPbRows||[]){lastPbId=Math.max(lastPbId,Number(x.id));if(x.status==="open"){const notional=Number(x.margin_usd)*Number(x.leverage);await send(`🟣 B플랜 · ${['ALGO','ETH','VET'].includes(x.symbol)?'보조 반전':'기본 패턴'} 신규 진입 🚀\n${x.symbol} ${side(x.side)}\n진입가: ${price(x.entry_price)}\n담보금: ${num(x.margin_usd)} USDT\n레버리지: ${x.leverage}x\n포지션 규모: ${num(notional)} USDT`)}else if(x.status==="rejected")await send(`🟣 B플랜 · 주문 거절/실패 ⚠️\n${x.symbol} ${side(x.side)}\n사유: ${String(x.reject_reason||"확인 필요").slice(0,500)}`)}
