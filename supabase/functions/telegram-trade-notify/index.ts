...
Deno.serve(async req=>{
 ...
 try{
  ...
  for(const x of closed||[]){if(x.closed_at>lastClosed)lastClosed=x.closed_at;const pnl=Number(x.net_pnl_usd);const roe=Number(x.margin_usd)>0&&Number.isFinite(pnl)?pnl/Number(x.margin_usd)*100:null;await send(`🔵 A플랜 · ${pnl>=0?"포지션 청산 ✅":"포지션 청산 🔻"} (거래소 정산)\n${x.symbol} ${side(x.side)}\n진입가: ${price(x.entry_price)}\n청산가: ${price(x.close_price??x.last_mark_price)}\n실현손익: ${pnl>=0?"+":""}${num(pnl)} USDT${roe==null?"":`\n담보수익률: ${roe>=0?"+":""}${num(roe)}%`}\n사유: ${x.close_reason||"전략/거래소 종료"}`)}

  // ---- B플랜 (plan_b_real_trades) ----
  // B settlement is asynchronous. Notify only verified settlement; track each trade independently.
  const {data:newPbRows,error:eb1}=await sb.from("plan_b_real_trades").select("id,symbol,side,status,margin_usd,leverage,entry_price,reject_reason").gt("id",lastPbId).order("id",{ascending:true});if(eb1)throw eb1;
  for(const x of newPbRows||[]){lastPbId=Math.max(lastPbId,Number(x.id));if(x.status==="open"){const notional=Number(x.margin_usd)*Number(x.leverage);await send(`🟣 B플랜 · 신규 진입 🚀\n${x.symbol} ${side(x.side)}\n진입가: ${price(x.entry_price)}\n담보금: ${num(x.margin_usd)} USDT\n레버리지: ${x.leverage}x\n포지션 규모: ${num(notional)} USDT`)}else if(x.status==="rejected")await send(`🟣 B플랜 · 주문 거절/실패 ⚠️\n${x.symbol} ${side(x.side)}\n사유: ${String(x.reject_reason||"확인 필요").slice(0,500)}`)}
  ...
  for(const x of closedPb||[]){if(x.net_pnl_usd==null||!Number.isFinite(Number(x.net_pnl_usd))||!(Number(x.close_price)>0))continue;const pnl=Number(x.net_pnl_usd);const roe=Number(x.margin_usd)>0&&Number.isFinite(pnl)?pnl/Number(x.margin_usd)*100:null;await send(`🟣 B플랜 · ${pnl>=0?"포지션 청산 ✅":"포지션 청산 🔻"}\n${x.symbol} ${side(x.side)}\n진입가: ${price(x.entry_price)}\n청산가: ${price(x.close_price)}\n실현손익: ${pnl>=0?"+":""}${num(pnl)} USDT${roe==null?"":`\n담보수익률: ${roe>=0?"+":""}${num(roe)}%`}`);const {error:sentError}=await sb.from("plan_b_real_trades").update({telegram_close_notified_at:new Date().toISOString()}).eq("id",x.id);if(sentError)throw sentError;}
