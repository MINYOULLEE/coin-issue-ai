...
async function closeTrade(account:any,trade:any,key:string,secret:string,reason:string){
 ...
 const owned=Math.min(actual,finite(trade.quantity));if(!(owned>0))throw Error("invalid owned close quantity");
 ...
 const client=`ma${String(account.id).replace(/-/g,"").slice(0,8)}c${trade.id}`.slice(0,40),raw=await signed(key,secret,"POST","/openApi/swap/v2/trade/order",{symbol,side:closeSide,positionSide,type:"MARKET",quantity:owned,clientOrderId:client,recvWindow:5000}),o=await confirmedOrder(key,secret,symbol,client,raw),price=finite(o.avgPrice||o.price||p?.markPrice),gross=(trade.side==="long"?1:-1)*(price-finite(trade.entry_price))*owned,fee=Math.abs(Number(o.commission??0))||((finite(trade.entry_price)+price)*owned*.0005),pnl=Number.isFinite(Number(o.profit))?Number(o.profit)-fee:gross-fee;
 await sql`update public.managed_bingx_trades set status='closed',close_price=${price},realized_pnl_usdt=${pnl},fee_usdt=${fee},closed_at=now(),close_reason=${reason},last_error=null,updated_at=now() where id=${trade.id}`;
}
