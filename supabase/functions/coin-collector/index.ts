...
// Keep the A ledger epoch stable so already-open automatic quantities are
// covered by C immediately and are resized at the normal daily boundary,
// rather than being churned through a close/re-entry solely for deployment.
const STRATEGY_EPOCH = "answer_mdd30_stage75_2026_09_08";
const IMMEDIATE_START_DATE_UTC = "2026-08-25";
const SIGNAL_MODEL_VERSION = "answer_mdd30_stage184_v1";
const STRATEGY_TYPES = ["answer_mdd30"];
const ANSWER_ASSETS = ["BTC","ETH","XRP","TRX","SOL"];
const ANSWER_CONFIG = {
  BTC:{weight:.125,low:.55,high:.75,lowX:.5,midX:1.5,highX:2},
  ETH:{weight:.275,low:.45,high:.70,lowX:0,midX:1,highX:1.5},
  XRP:{weight:.10,low:.45,high:.60,lowX:0,midX:.5,highX:1.5},
  TRX:{weight:.40,low:.45,high:.60,lowX:0,midX:1,highX:1},
  SOL:{weight:.30,low:.45,high:.60,lowX:0,midX:1,highX:1},
};
...
function sampleStd(values){
  const z=values.filter(Number.isFinite);if(z.length<2)return 0;const mean=z.reduce((a,b)=>a+b,0)/z.length;
  return Math.sqrt(z.reduce((a,b)=>a+(b-mean)**2,0)/(z.length-1));
}
function populationStd(values){const z=values.filter(Number.isFinite);if(!z.length)return 0;const mean=z.reduce((a,b)=>a+b,0)/z.length;return Math.sqrt(z.reduce((a,b)=>a+(b-mean)**2,0)/z.length)}
function median(values){const z=values.filter(Number.isFinite).sort((a,b)=>a-b);if(!z.length)return 0;const m=Math.floor(z.length/2);return z.length%2?z[m]:(z[m-1]+z[m])/2}
function correlation(a,b){if(a.length!==b.length||a.length<2)return 0;const ma=a.reduce((s,x)=>s+x,0)/a.length,mb=b.reduce((s,x)=>s+x,0)/b.length;let n=0,da=0,db=0;for(let i=0;i<a.length;i++){const x=a[i]-ma,y=b[i]-mb;n+=x*y;da+=x*x;db+=y*y}return da&&db?n/Math.sqrt(da*db):0}
function percentileRank(values,current){
  const z=values.filter(Number.isFinite).sort((a,b)=>a-b);if(!z.length)return .5;
  const below=z.filter(x=>x<current).length,equal=z.filter(x=>x===current).length;
  return (below+(equal+1)/2)/z.length;
}
function answerFeatures(asset,rows){
  if(!Array.isArray(rows)||rows.length<721)return null;
  const z=rows.slice(-1000),o=z.map(x=>Number(x[1])),h=z.map(x=>Number(x[2])),l=z.map(x=>Number(x[3])),c=z.map(x=>Number(x[4])),v=z.map(x=>Number(x[5])),q=z.map(x=>Number(x[7]||0));
  if([...o,...h,...l,...c,...v,...q].some(x=>!Number.isFinite(x)))return null;
  const last=c.length-1,ret=[];for(let i=1;i<c.length;i++)ret.push(c[i]/c[i-1]-1);
  const values:Record<string,number>={};
  for(const n of [1,3,6,12,24,72,168,336,720])values[`return_${n}h`]=c[last]/c[last-n]-1;
  for(const [fast,slow] of [[12,72],[24,168],[72,336],[168,720]])values[`ema_gap_${fast}_${slow}`]=ema(c,fast)/ema(c,slow)-1;
  for(const n of [24,72,168,336,720]){
    const priorH=h.slice(last-n,last),priorL=l.slice(last-n,last),hi=Math.max(...priorH),lo=Math.min(...priorL);
    values[`channel_${n}`]=(c[last]-lo)/(hi-lo||1);
    values[`vol_${n}`]=sampleStd(ret.slice(-n))*Math.sqrt(365.25*24);
  }
  for(const n of [14,72,168]){
    const rr=ret.slice(-n),gain=rr.reduce((a,b)=>a+Math.max(b,0),0)/n,loss=rr.reduce((a,b)=>a+Math.max(-b,0),0)/n;
    values[`rsi_${n}`]=loss?100-100/(1+gain/loss):100;
  }
  const tr=[];for(let i=1;i<c.length;i++)tr.push(Math.max(h[i]-l[i],Math.abs(h[i]-c[i-1]),Math.abs(l[i]-c[i-1])));
  for(const n of [24,72,168])values[`atr_${n}`]=tr.slice(-n).reduce((a,b)=>a+b,0)/n/c[last];
  values.body=(c[last]-o[last])/o[last];values.upper_wick=(h[last]-Math.max(o[last],c[last]))/c[last];values.lower_wick=(Math.min(o[last],c[last])-l[last])/c[last];
...
async function fetchMarket(){
  ...
  const entries=await Promise.all(tickers.map(async t=>{
    ...
    const answer=ANSWER_ASSETS.includes(s)?answerFeatures(s,hourRows):null;
...
async function triggerClose(signal,reason){
  if(!INTERNAL_TRADE_SECRET)return false;
  try{
    ...
    const j=await r.json().catch(()=>({}));if(!j.ok||!j.closed)console.error("strategy close not applied:",signal.symbol,j.error||j.skipped);return !!(j.ok&&j.closed);
  }catch(e){console.error("triggerClose failed:",e instanceof Error?e.message:String(e));return false}
}
async function triggerMdd30Resize(signal,baseExposure){
  if(!INTERNAL_TRADE_SECRET)return {ok:false,error:"internal trade secret missing"};
  try{
    const r=await fetch(PROJECT_URL+"/functions/v1/bingx-order-execute",{method:"POST",headers:{"Content-Type":"application/json","x-internal-key":INTERNAL_TRADE_SECRET},body:JSON.stringify({action:"resize_mdd30",id:signal.id,base_exposure_multiplier:baseExposure})});
    return await r.json().catch(()=>({ok:false,error:"invalid resize response"}));
  }catch(e){return {ok:false,error:e instanceof Error?e.message:String(e)}}
}
async function triggerMdd30RallyGuard(signal,targetFraction,phase){
  if(!INTERNAL_TRADE_SECRET)return {ok:false,error:"internal trade secret missing"};
  try{
    const r=await fetch(PROJECT_URL+"/functions/v1/bingx-order-execute",{method:"POST",headers:{"Content-Type":"application/json","x-internal-key":INTERNAL_TRADE_SECRET},body:JSON.stringify({action:"rally_guard_mdd30",id:signal.id,target_fraction:targetFraction,phase})});
    return await r.json().catch(()=>({ok:false,error:"invalid rally guard response"}));
  }catch(e){return {ok:false,error:e instanceof Error?e.message:String(e)}}
}
...
function positionPlan(entry,invalidation,confidence,type,microVol,availableMargin=1000,equity=1000,market={},researchExposure=null){
  if(type===CANDIDATE_A_BIG||type===CANDIDATE_A_SMALL){
    const exposure=Number(researchExposure??(type===CANDIDATE_A_BIG?(market.trend_strength_percentile_90d>=90?8:1):.5)),leverage=5;
  ...
  const vol=Number(microVol||0),baseLeverage=confidence>=78?5:confidence>=70?3:2;
  const macroMixed=market.trend_4h==="neutral"||market.trend_1d==="neutral"||market.trend_4h!==market.trend_1d;
  const leverage=Math.min(baseLeverage,5);
...
async function manageSignals(market,old){
  ...
  for(const s of active){
    const price=Number(market[s.symbol]?.price||s.entry_price),closedHourAt=Number(market[s.symbol]?.suite_setup?.closed_at||0),newHour=closedHourAt>Date.parse(s.last_reviewed_at||s.created_at),reviewPrice=Number(market[s.symbol]?.suite_setup?.close||price),answerStrategy=s.signal_type==="answer_mdd30",stopEnabled=!answerStrategy&&s.signal_type!=="strategy_f",stopped=newHour&&stopEnabled&&(s.side==="long"?reviewPrice<=Number(s.invalidation_price):reviewPrice>=Number(s.invalidation_price)),expired=!answerStrategy&&Date.now()>=Date.parse(s.expires_at),regimeExit=s.signal_type==="strategy_d"&&String(market.candidate_a?.regime)!=="transition";
  ...
  async function enter(symbol,type,side,exposure,leverage,hours,reasons,stopPct,modelConfidence=80,manualImmediate=false){
    ...
    // trade_signals.account_equity_usd is NOT NULL. The executor still sizes
    // the real order from the live BingX equity; these values are an auditable
    // signal-time estimate and must never be null or the entire collector run
  ...
  if(candidates.a_rally_guard?.version!=="stage135"){
    const anchorClosedAt=Math.min(...ANSWER_ASSETS.map(x=>Number(market[x]?.suite_setup?.daily_a_anchor_closed_at||0)));
    const anchorPrices=Object.fromEntries(ANSWER_ASSETS.map(x=>[x,Number(market[x]?.suite_setup?.daily_a_anchor_price||0)]));
    if(anchorClosedAt>0&&ANSWER_ASSETS.every(x=>anchorPrices[x]>0))candidates.a_rally_guard={version:"stage135",anchor_closed_at:anchorClosedAt,anchor_prices:anchorPrices,last_evaluated_closed_at:anchorClosedAt,phase:0,streak:0,bootstrapped:true,updated_at:nowIso};
  }
  // Stage135: after the daily A anchor, evaluate each newly completed common hour once.
  // Two consecutive qualifying closes are required. Only A ETH/XRP/SOL shorts are reduced.
  const rally:any=candidates.a_rally_guard;
  const commonClosedAt=Math.min(...ANSWER_ASSETS.map(x=>Number(market[x]?.suite_setup?.closed_at||0)));
  if(rally?.version==="stage135"&&commonClosedAt>Number(rally.anchor_closed_at||0)&&commonClosedAt>Number(rally.last_evaluated_closed_at||0)&&Number(rally.phase||0)<2){
    const anchors=rally.anchor_prices||{},returns=Object.fromEntries(ANSWER_ASSETS.map(x=>[x,Number(market[x]?.suite_setup?.close||0)/Number(anchors[x]||0)-1]));
    const phase=Number(rally.phase||0),btcCut=phase===0?.005:.02,assetCut=phase===0?.0275:.0375;
    const valid=ANSWER_ASSETS.every(x=>Number.isFinite(returns[x])&&Number(anchors[x])>0);
    const breadth=valid?ANSWER_ASSETS.filter(x=>returns[x]>=assetCut).length:0,raw=valid&&returns.BTC>=btcCut&&breadth>=3;
    rally.last_evaluated_closed_at=commonClosedAt;rally.streak=raw?Number(rally.streak||0)+1:0;rally.updated_at=nowIso;
    rally.last_check={closed_at:commonClosedAt,btc_return:returns.BTC,breadth,asset_cut:assetCut,raw_trigger:raw};
    if(rally.streak>=2){
      ...
      rally.last_action={phase:phase+1,target_fraction:targetFraction,closed_at:commonClosedAt,results,completed_at:nowIso};
      if(complete){rally.phase=phase+1;rally.streak=0;rally.retry_required=false}else{rally.retry_required=true;rally.last_evaluated_closed_at=commonClosedAt-1}
    }
    candidates.a_rally_guard=rally;
  }
  // Stage184 C: completed common hours only. C intervenes only when short-term
  // volatility is exceptional, all five assets are highly synchronized, and
  // several currently held A directions are simultaneously adverse.
  ...
  if(commonClosedAt>Number(cState.last_evaluated_closed_at||0)){
    ...
    if(valid){
      ...
      const targets=[...new Map([...(instant.length>=2?instant:[]),...(persistent.length>=3?persistent:[]),...(broad.length>=3?broad:[])].map(x=>[x.id,x])).values()];
      const results=[];
      if(qualifies){
  ...
  return {active,candidates,health,recent,cooldowns,account,regime:{mode:"answer_mdd30",strategy:"BTC·ETH·XRP·TRX·SOL independent A+B · MDD30",market_regime:"answer_mdd30",pending:null,carried_positions:legacyActive.map(x=>({id:x.id,symbol:x.symbol,side:x.side,signal_type:x.signal_type})),checked_at:nowIso}};
}
...
Deno.serve(async req=>{
  ...
  try{
    ...
    if(requestBody?.action==="close_all_a"){
      const active=await activeSignals();
      const targets=active.filter((s:any)=>s.signal_type==="answer_mdd30");
      const results=[];
      for(const signal of targets){
        const closed=await triggerClose(signal,"A Stage75 전환 전 사용자 승인 전량 청산");
        results.push({id:signal.id,symbol:signal.symbol,side:signal.side,closed});
      }
      return Response.json({ok:results.every((x:any)=>x.closed),action:"close_all_a",results});
    }
    if(requestBody?.action==="audit_a"){
      const r=await fetch(PROJECT_URL+"/functions/v1/bingx-order-execute",{method:"POST",headers:{"Content-Type":"application/json","x-internal-key":INTERNAL_TRADE_SECRET},body:JSON.stringify({action:"audit"})});
      return Response.json(await r.json().catch(()=>({ok:false,error:"invalid audit response"})),{status:r.status});
    }
    const old=await current();
    const market=await fetchMarket();
    const signalState=await manageSignals(market,old);
    // News retired by owner. Keep all market/trading state; omit stale news from live snapshot.
