...
export function normalizeBOrder(data) {
 ...
 if(qty>0 && Number.isFinite(price)&&price>0) return {status:status==='FILLED'?'filled':'partially_filled',quantity:qty,price,orderId:String(id||'')};
...
export function createBExchange({apiKey,secret,parse=JSON.parse,fetcher=fetch,liveAuthorized=()=>false}) {
 ...
 async function request(method,path,params={}) {
  ...
  if(method!=='GET' && (!standard.live_ready||!liveAuthorized()||path!=='/openApi/swap/v2/trade/order'))throw Error('B live transport locked');
 ...
 return {read:(path,params)=>request('GET',path,params),
  ...
  async submit(order){if(order.plan!=='B'||!standard.symbols[order.symbol]||!order.clientOrderId.startsWith('pb16-')||!['long','short'].includes(order.side))throw Error('invalid B order');
   ...
   return normalizeBOrder(await request('POST','/openApi/swap/v2/trade/order',{symbol:order.symbol+'-USDT',side,positionSide:order.side.toUpperCase(),type:'MARKET',quantity:order.quantity,clientOrderId:order.clientOrderId}));
  }
 };
}
