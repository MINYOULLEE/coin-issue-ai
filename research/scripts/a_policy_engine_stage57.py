...
def a_replay(series,maps,fs,times,rule,start=None,end=None,cost_mult=1):
    ...
    for t in ts:
        ...
        for sym,p in list(pos.items()):
            reason=None
            if POLICY.startswith('hold_') and t-p['t']>=int(POLICY.split('_')[1])*H:
                reason='time_limit'
            elif POLICY=='weak_move' and t in fs[sym] and fs[sym][t]['move']*p['side']<0 and fs[sym][t]['eff']>.35:
                reason='opposing_completed_6h_move'
            elif POLICY=='weak_mean' and t in fs[sym] and t-H in series[sym] and p['side']*(series[sym][t-H]['c']-fs[sym][t]['mean'])<0:
                reason='completed_close_cross_mean'
            if reason:
                close(sym,t,series[sym][t]['o'],reason)
                blocked.add(sym)
        ...
        for s,w in wants.items():
            if w and s not in pos:
                ...
                if POLICY=='confirm_1h' and w*(series[s][t-H]['c']/series[s][t-H]['o']-1)<=0:continue
                if POLICY=='confirm_6h' and w*fs[s][t]['move']<=0:continue
