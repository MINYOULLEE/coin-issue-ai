// Offline regression tests for the current B execution collision rules.
...
function fixture(status='closing'){
 const tables={plan_b_trading_state:[{id:'singleton',strategy_id:'b_profit_lock_stage66',enabled:true,test_mode:false}],plan_b_execution_intents:[{signal_id:1,status,symbol:'AVAX',reserved_usd:90,close_attempt:1,close_client_order_id:'pb66-1-c1'}],plan_b_real_trades:[{id:1,signal_id:1,status:'open',symbol:'AVAX',side:'long',quantity:2,bingx_order_id:'entry',client_order_id:'pb66-1',plan_b_signals:{expires_at:new Date(NOW-1).toISOString(),strategy_id:'b_profit_lock_stage66',strategy_params:null}}]};
...
test('archived Stage16 signals cannot enter the current Stage66 runtime',()=>{for(const symbol of ['ALGO','ETH','VET'])assert.equal(eligible({symbol,strategy_id:'b_reserved_margin_stage16',side:'long',leverage:3,hold_hours:1,confirmed_at:new Date(NOW).toISOString(),entry_deadline:new Date(NOW+60000).toISOString()},NOW),false);});
