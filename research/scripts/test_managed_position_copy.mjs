...
import {copyDelta,copyKeys,normalizedPositions,positionKey,targetQuantity} from '../../supabase/functions/_shared/managed_position_copy.mjs';
...
test('stale open ledger remains in reconciliation after both exchange positions disappear',()=>{
 const keys=copyKeys([],[],[{symbol:'BTC',side:'long'}]);
 assert.deepEqual([...keys],['BTC-USDT:LONG']);
});
