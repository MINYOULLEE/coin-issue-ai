...
function fixture(order={orderId:'1234567890123456789',executedQty:2,avgPrice:101,status:'FILLED'},options={}){
 const calls=[],writes=[];let reservation=null,trade=options.closed?{id:9,status:'closed'}:null,sourceReads=0;
 const db=async(path,init={})=>{const method=init.method||'GET',body=init.body&&JSON.parse(init.body);writes.push({path,method,body});
  ...
  if(path.startsWith('trade_signals')){sourceReads++;return [{id:101,symbol:'BTC',side:'long',signal_type:'answer_mdd30',strategy_epoch:undefined,status:options.closeBeforeSubmit&&sourceReads>1?'neutral':options.inactive?'neutral':'active'}];}
...
test('A inactive source cannot reach leverage or order',async()=>{const f=fixture(undefined,{inactive:true});assert.equal((await f.cycle.submit(p)).skipped,'A source signal no longer active');assert.equal(f.calls.length,0);assert.equal(f.reservation,null);});
test('A source closed during preflight cannot reach live order',async()=>{const f=fixture(undefined,{closeBeforeSubmit:true});assert.equal((await f.cycle.submit(p)).skipped,'A source signal closed before submit');assert.equal(f.calls.filter(c=>c.path.endsWith('/order')).length,0);assert.equal(f.reservation,null);});
