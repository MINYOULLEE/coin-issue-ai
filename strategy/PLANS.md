...
2026-09-01 뉴스 제거 승인: `NEWS_RETIREMENT_20260901.md`. 목적지 명시 사용자 승인 후 뉴스 제거 수집기 v77·알림 v30 배포 완료. A/B 매매 규칙·스위치는 변경하지 않았다. 뉴스 출처의 과거 403은 복구가 아니라 기능 폐기로 종료한다.

2026-09-01 반복 뉴스 알림·로컬 수집기 점검: `ALERT_STARTUP_REPAIR_20260901.md`. 뉴스에만 지속 실패/복구 확인을 적용하며 매매 오류 알림은 즉시 유지. 로컬 작업 종료 후 사용자 관리자 승인으로 `Coin Issue AI Collector` 자동시작 비활성화 완료(`Enabled=false` 재조회 확인). 클라우드 수집기와 A/B 실거래 설정은 변경하지 않았다.
