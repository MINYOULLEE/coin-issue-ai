...
채택 기준 고정과 운영 적용은 별개다. 2026-08-30 Stage16 비활성 DB, B 신호 미리보기 함수 v1, 텔레그램 v12 배포 완료. 계좌 조회 함수 수정본은 상세 기록 공개 보안 검토로 배포 차단되어 기존 v5 유지. B의 live_ready=false, enabled=false, test_mode=true 유지. 모의 전용 원자적 담보 예약/선점 RPC와 주문 생명주기 모의 테스트는 구현했지만, 실제 주문 실행기·체결동기화·거래소 계약규격 연결은 완료하지 않았다. 버튼만 켜면 실거래 준비 완료라고 설명하지 않는다. 상세 상태는 `B_PRELIVE_STATUS.md` 참조.
